...
.OUTPUTS
    Hashtable
#>
function Get-letheanSDPHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-letheanSDPConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-letheanSDPStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-letheanSDPUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-letheanSDPUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-letheanSDPStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-letheanSDPKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-letheanSDPRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-letheanSDPECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Depricated
    <#$cryptographicScheme = Get-letheanSDPCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-letheanSDPRSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "letheanSDPRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-letheanSDPECDSASignature {
    param(
...
.Outputs
    String
#>
Function Get-letheanSDPStringHash {
    param(
        [Parameter(Mandatory = $true)]
        [AllowEmptyString()]
        [string]$String,
...
Integer
#>
function Get-letheanSDPUnixTime {
    param(
        [Parameter(Mandatory = $true)]
        [DateTime]$Date,
        [Parameter(Mandatory = $false)]
...
function Get-letheanSDPCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-letheanSDPKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
